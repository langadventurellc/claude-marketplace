...
description: Performs technical research and analysis of a problem or proposed change without implementing anything. Use when asked to "analyze this", "research this change", "investigate", "what would be involved", "where would changes occur", "scope this work", "technical discovery", or before planning complex work to understand the technical landscape. Produces a structured in-chat report covering affected code, external research, risks, and recommendations.
allowed-tools:
  - Task
  - Glob
  - Grep
...
Perform technical research and analysis to understand a problem, question, or proposed change. Produce a structured report without any code implementation.

## Goal

Investigate the codebase and relevant context to provide findings that inform decision-making, planning, or subsequent ticket creation. This skill answers "what would be involved?" without actually doing the work.

## Required Inputs

- **Problem or Question**: Description of what to analyze (e.g. "How would we add user authentication?" or "Why is the API slow?")
- **Additional Context** (optional): Constraints, related work, or specific areas to focus on
...
- **Broad exploration**: For wide-ranging searches across unfamiliar or large codebases, delegate via the `Task` tool with `subagent_type="Explore"` — Explore is a fast, read-only agent well-suited for surveying multiple areas in parallel. Use it when a simple Glob/Grep won't suffice; prefer direct Glob/Grep for targeted lookups where you already know what to search for.

### 3. Research External Context

When the problem involves libraries, APIs, or concepts outside the codebase:

- **Use web search tools**: query WebSearch, WebFetch, or a search MCP server (if available) for up-to-date documentation, best practices, or known issues
- **Check documentation**: fetch relevant docs via WebFetch when URLs are known
...
Provide a structured report in-chat:
...
#### External Research
[Relevant findings from web search, documentation, or other sources — omit if not applicable]
