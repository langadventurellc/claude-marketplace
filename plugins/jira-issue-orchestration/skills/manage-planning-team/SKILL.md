...
description: Internal skill. Runs inside the planning sub-session. Invokes orchestration-investigate-jira-issue to produce a requirements artifact, then invokes create-trellis-issues to create Trellis issues from it. Signals completion to the conductor when done.
allowed-tools:
  - AskUserQuestion
  - Skill
...
Invoke `orchestration-investigate-jira-issue` via the `Skill` tool, passing the Jira issue key received from the conductor's instructions.

```
Skill({ name: "orchestration-investigate-jira-issue", input: "<JIRA_KEY>" })
```
